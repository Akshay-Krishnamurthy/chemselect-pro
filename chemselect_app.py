...
Markush drawings — open the SciFinder record and match directly by label.
    ...
    if not RDKIT_OK: st.error("RDKit required.")
        ...
        if st.session_state.markush_summary is not None:
            ...
            if len(sel_rg):
                # Show one row per R-group with all its variants
                pivot_rows = []
                for rg_name, grp in sel_rg.groupby("RGroup"):
                    smis = " | ".join(sorted(grp["RGroup_SMILES"].dropna().astype(str).tolist()))
                    sf   = grp["SciFinder_Label"].iloc[0] if "SciFinder_Label" in grp.columns else ""
                    pivot_rows.append({"R-Group": rg_name, "SciFinder_Label": sf,
                                       "Variants": smis, "N_Variants": grp["RGroup_SMILES"].notna().sum()})
                st.dataframe(pd.DataFrame(pivot_rows), use_container_width=True)
            else:
...
Match these labels directly to the Markush drawing in SciFinder patent records.
            ...
            c2.download_button("⬇ R-Group Table (SciFinder Labels)",rg.to_csv(index=False).encode(),"rgroups_scifindr.csv","text/csv")
